...
class CustomGraformer(nn.Module):
    ...
    def forward(self, source, src_mask, target, tgt_mask) -> torch.Tensor:
        ...
        memory = self.k_layer_encoder_stack.forward(masked_encoder_output, src_key_padding_mask=(1-src_mask).to(dtype=causal_decoder_output.dtype))

        output = self.k_layer_decoder_stack.forward(causal_decoder_output, memory, tgt_key_padding_mask=(1-tgt_mask).to(dtype=causal_decoder_output.dtype),
    ...
    def encode(self, src, src_mask):
        ...
        return self.k_layer_encoder_stack.forward(masked_encoder_output, src_key_padding_mask=(1-src_mask).half())
    ...
    def decode(self, tgt, memory, tgt_mask):
        ...
        return causal_decoder_output, self.k_layer_decoder_stack.forward(causal_decoder_output, memory,
                                                                         tgt_key_padding_mask=(1-tgt_mask).half(),
